...
contract TornadoStakingRewards is Initializable, EnsResolve {
    ...
    ITornadoGovernance public Governance;
    ...
    function updateRelayerRegistry(address _relayerRegistry) external{
        relayerRegistry = _relayerRegistry;
    }

    function updateGovernance(address _governance) external{
        Governance = ITornadoGovernance(_governance);
    }
    ...
    function getReward() external {
        uint256 rewards = _updateReward(
            msg.sender,
            Governance.lockedBalance(msg.sender)
        );
    ...
    function addBurnRewards(uint256 amount) external {
        require(
            msg.sender == address(Governance) || msg.sender == relayerRegistry,
            "unauthorized"
        );
        accumulatedRewardPerTorn = accumulatedRewardPerTorn.add(
            amount.mul(ratioConstant).div(
                torn.balanceOf(address(Governance.userVault()))
            )
        );
    }
    ...
    function _updateReward(
        ...
        if (amountLockedBeforehand != 0)
            claimed = (
                accumulatedRewardPerTorn.sub(
                    accumulatedRewardRateOnLastUpdate[account]
                )
            ).mul(amountLockedBeforehand).div(ratioConstant);
    ...
    function checkReward(
        ...
        if (amountLocked != 0)
            ...
            ).mul(amountLocked).div(ratioConstant);
