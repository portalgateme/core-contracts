...
contract Configuration {
  ...
  function setExecutionDelay(uint256 executionDelay) external  {
    EXECUTION_DELAY = executionDelay;
  }

  function setExecutionExpiration(uint256 executionExpiration) external  {
  ...
  function setClosingPeriod(uint256 closingPeriod) external  {
    CLOSING_PERIOD = closingPeriod;
  }

  function setVoteExtendTime(uint256 voteExtendTime) external  {
