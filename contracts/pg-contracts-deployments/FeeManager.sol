...
contract FeeManager is EnsResolve {
    ...
    constructor(address _torn, address _registry) public {
        torn = _torn;        
    ...
    function calculatePoolFee(
        ...
        token = token == IERC20(0) && !isERC20
            ? IERC20(UniswapV3OracleHelper.WETH)
            : token; // for eth instances
     ...
     */
    function setUniswapTornPoolSwappingFee(
     ...
     * */
    function setPeriodForTWAPOracle(uint32 newPeriod) external {
     ...
     * */
    function setUpdateFeeTimeLimit(uint24 newLimit) external {
    ...
    function feeDeviations() public view returns (Deviation[] memory results) {
        ITornadoInstance[] memory instances = registry
        ...
        for (uint256 i = 0; i < instances.length; i++) {
            ...
            if (marketFee != 0) {
                deviation =
                    int256((instanceFee[instances[i]] * 1000) / marketFee) -
                    1000;
            }

            results[i] = Deviation({
                instance: address(instances[i]),
                deviation: deviation
            });
        }
    }
